...
crudini --set /etc/php/7.1/fpm/php.ini PHP max_execution_time ${PHP_MAX_EXECUTION_TIME}
crudini --set /etc/php/7.1/fpm/php.ini PHP max_input_time ${PHP_MAX_INPUT_TIME}
crudini --set /etc/php/7.1/fpm/php.ini PHP max_input_vars ${PHP_MAX_INPUT_VARS}
crudini --set /etc/php/7.1/fpm/php.ini PHP memory_limit ${PHP_MEMORY_LIMIT}
crudini --set /etc/php/7.1/fpm/php.ini PHP display_errors ${PHP_DISPLAY_ERRORS}
crudini --set /etc/php/7.1/fpm/php.ini PHP display_startup_errors ${PHP_DISPLAY_STARTUP_ERRORS}
crudini --set /etc/php/7.1/fpm/php.ini PHP log_errors ${PHP_LOG_ERRORS}
crudini --set /etc/php/7.1/fpm/php.ini PHP log_errors_max_len ${PHP_LOG_ERRORS_MAX_LEN}
crudini --set /etc/php/7.1/fpm/php.ini PHP ignore_repeated_errors ${PHP_IGNORE_REPEATED_ERRORS}
crudini --set /etc/php/7.1/fpm/php.ini PHP ignore_repeated_source ${PHP_IGNORE_REPEATED_SOURCE}
crudini --set /etc/php/7.1/fpm/php.ini PHP report_memleaks ${PHP_REPORT_MEMLEAKS}
crudini --set /etc/php/7.1/fpm/php.ini PHP post_max_size ${PHP_POST_MAX_SIZE}
crudini --set /etc/php/7.1/fpm/php.ini PHP default_charset ${PHP_DEFAULT_CHARSET}
crudini --set /etc/php/7.1/fpm/php.ini PHP file_uploads ${PHP_FILE_UPLOADS}
crudini --set /etc/php/7.1/fpm/php.ini PHP upload_max_filesize ${PHP_UPLOAD_MAX_FILESIZE}
crudini --set /etc/php/7.1/fpm/php.ini PHP max_file_uploads ${PHP_MAX_FILE_UPLOADS}
crudini --set /etc/php/7.1/fpm/php.ini PHP allow_url_fopen ${PHP_ALLOW_URL_FOPEN}

# Xdebug Configuration
crudini --set /etc/php/7.1/fpm/php.ini xdebug xdebug.remote_enable ${XDEBUG_REMOTE_ENABLE}
crudini --set /etc/php/7.1/fpm/php.ini xdebug xdebug.remote_connect_back ${XDEBUG_REMOTE_CONNECT_BACK}
crudini --set /etc/php/7.1/fpm/php.ini xdebug xdebug.remote_host ${XDEBUG_REMOTE_HOST}
crudini --set /etc/php/7.1/fpm/php.ini xdebug xdebug.remote_handler ${XDEBUG_REMOTE_HANDLER}
crudini --set /etc/php/7.1/fpm/php.ini xdebug xdebug.remote_port ${XDEBUG_REMOTE_PORT}
crudini --set /etc/php/7.1/fpm/php.ini xdebug xdebug.remote_autostart ${XDEBUG_REMOTE_AUTOSTART}
crudini --set /etc/php/7.1/fpm/php.ini xdebug xdebug.extended_info ${XDEBUG_EXTENDED_INFO}

# Worker pool settings can only be configured via environment variables, not via .tok/config.yml
crudini --set /etc/php/7.1/fpm/pool.d/www.conf www pm.max_children ${WWW_PM_MAX_CHILDREN:-30}
crudini --set /etc/php/7.1/fpm/pool.d/www.conf www pm.start_servers ${WWW_PM_START_SERVERS:-5}
crudini --set /etc/php/7.1/fpm/pool.d/www.conf www pm.min_spare_servers ${WWW_PM_MIN_SPARE_SERVERS:-5}
crudini --set /etc/php/7.1/fpm/pool.d/www.conf www pm.max_spare_servers ${WWW_PM_MAX_SPARE_SERVERS:-5}
crudini --set /etc/php/7.1/fpm/pool.d/www.conf www pm.process_idle_timeout ${WWW_PM_PROCESS_IDLE_TIMEOUT:-10s}
